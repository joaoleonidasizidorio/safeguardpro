...
const InventoryTab: React.FC<{ companyId: string }> = ({ companyId }) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            ...
            alert(`Erro ao salvar EPI: ${errorData.error || 'Erro desconhecido'}`);
        }
    };
        ...
        <div>
            <div className="flex justify-between items-center mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Para cadastrar ou editar, acesse o módulo <strong>Empresas {'>'} Colaboradores</strong>.</p>
                </div>
            </div>
            <div className="bg-white dark:bg-surface-dark rounded shadow overflow-hidden">
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {employees.map(emp => (
                            <tr key={emp.id}>
                                ...
                                <td className="p-3">{new Date(emp.admission_date).toLocaleDateString()}</td>
...
export const EPIManagement: React.FC = () => {
    ...
    if (loading) return <div className="p-10 text-center animate-pulse text-gray-500">Carregando dados de EPI...</div>;
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div>
                    {companies.length > 0 ? (
                        <select
                            className="p-2 border rounded-lg bg-white dark:bg-surface-dark dark:border-gray-700 font-bold text-sm"
                        ...
                        <div className="text-xs text-orange-500 font-bold bg-orange-50 dark:bg-orange-900/20 px-3 py-1 rounded-full border border-orange-200">
            ...
            <div className="flex border-b border-gray-200 dark:border-gray-800">
                ...
                <button
                    ...
                    className={`px-6 py-3 font-bold text-sm border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'employees' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                >
            ...
            <div className="pt-4">
                {selectedCompanyId ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-gray-50 dark:bg-surface-dark/30 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-800">
                        <span className="material-symbols-outlined text-6xl text-gray-300 mb-4">business</span>
                        <h3 className="text-xl font-bold text-gray-500">Nenhuma empresa selecionada</h3>
                        <p className="text-sm text-gray-400 mt-2 mb-6">Você precisa cadastrar uma empresa primeiro para gerenciar seus EPIs.</p>
                        <button 
                            className="bg-primary text-background-dark px-6 py-2 rounded-xl font-black text-sm hover:scale-105 transition-all shadow-lg shadow-primary/20"
                            onClick={() => window.location.href = '#companies'} // Assuming hash routing or similar
                        >

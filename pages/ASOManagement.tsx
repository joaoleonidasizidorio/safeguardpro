...
export const ASOManagement: React.FC = () => {
    ...
    const fetchASOs = async () => {
        setLoading(true);
        try {
            let url = `${API_BASE_URL}/api/asos?page=${page}&limit=${limit}`;
    ...
    const handleDownloadPDF = async (id: number) => {
        try {
            const res = await authFetch(`${API_BASE_URL}/api/asos/${id}/pdf`, {
                method: 'POST'
            });
            if (res.ok) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await authFetch(`${API_BASE_URL}/api/asos/${id}`, { method: 'DELETE' });
            if (res.ok) {
                fetchASOs();
    ...
    const handleBulkDelete = async () => {
        ...
        for (const id of selectedIds) {
            await authFetch(`${API_BASE_URL}/api/asos/${id}`, { method: 'DELETE' });
        }
        setSelectedIds([]);
        fetchASOs();
    };
    ...
    const handleEdit = async (id: number) => {
        try {
            setLoading(true);
            const res = await authFetch(`${API_BASE_URL}/api/asos/${id}?t=${Date.now()}`, {

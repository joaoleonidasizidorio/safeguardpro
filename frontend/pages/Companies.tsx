...
export const Companies: React.FC = () => {
  ...
  const fetchSubData = async (companyId: string) => {
    try {
      ...
      const [unitsData, historyData] = await Promise.all([unitsRes.json(), historyRes.json()]);
      setUnits(unitsData);
  ...
  const handleSaveCompany = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const response = await fetch(`${API_BASE_URL}/api/companies/${editingId}`, {
          method: 'PUT',
                      ...
                      <button
                        onClick={async () => {
                          if (confirm('Excluir unidade?')) {
                            await fetch(`${API_BASE_URL}/api/units/${unit.id}`, { method: 'DELETE' });
                        ...
                        <button
                          onClick={async () => {
                            if (confirm('Excluir registro?')) {
                              await fetch(`${API_BASE_URL}/api/service-history/${item.id}`, { method: 'DELETE' });
                              fetchSubData(selectedCompany.id);
                            }
                          }}
                          className="p-1 text-gray-400 hover:text-red-500"
    ...
    <div className="space-y-6" onClick={() => setOpenActionId(null)}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-gray-500 dark:text-gray-400">Gerencie e acompanhe todos os registros de segurança e visitas dos clientes.</p>
        </div>
        <button

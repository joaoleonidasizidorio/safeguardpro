...
interface Incident {
    ...
    type: 'Incidente' | 'Acidente' | 'Quase-acidente';
    date: string;
    location: string;
    description: string;
...
export const IncidentManagement: React.FC = () => {
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (const file of files) {
            ...
            const res = await fetch(`${API_BASE_URL}/api/incidents/upload`, {
                method: 'POST',
                body: formData
            });
            if (res.ok) {
    ...
    const handleOpenInvestigation = (inc: Incident) => {
        ...
        setInvResult(inc.investigation_result || '');
    ...
    const handleSaveInvestigation = async () => {
        ...
        const res = await fetch(`${API_BASE_URL}/api/incidents/${selectedIncident.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ investigation_result: invResult, status: invStatus })
        });
    ...
    const handleAddActionPlan = async (e: React.FormEvent) => {
        ...
        const res = await fetch(`${API_BASE_URL}/api/incidents/${selectedIncident.id}/actions`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Investigation Modal */}
            <Modal isOpen={isInvModalOpen} onClose={() => setIsInvModalOpen(false)} title="Investigação de Incidente">
                {selectedIncident && (
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm mb-1 font-bold text-primary">Conclusão da Investigação / Causa Raiz</label>
                                <textarea
                                    rows={4}
                                    placeholder="Descreva os resultados da investigação..."
                                    className="w-full p-2 border rounded dark:bg-background-dark dark:border-gray-700"
                                    value={invResult}
                            ...
                            <div className="flex items-center gap-4">
                                <label className="text-sm font-bold">Status Atual:</label>
                                <select
                                    className="p-2 border rounded dark:bg-background-dark dark:border-gray-700 text-sm"
                                    value={invStatus}
                        ...
                        <button
                            ...
                            className="w-full bg-primary text-black p-3 rounded font-bold shadow-lg"
                        >
                            Salvar Investigação
                        </button>
                    </div>

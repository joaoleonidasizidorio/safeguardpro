...
export const ASOForm: React.FC<ASOFormProps> = ({ onSuccess, onCancel, initialData }) => {
    ...
    const fetchInitialData = async () => {
        try {
            const [compRes, examRes, riskRes] = await Promise.all([
                authFetch('http://localhost:7000/api/companies'),
                authFetch('http://localhost:7000/api/exam-types'),
                authFetch('http://localhost:7000/api/risks') // Make sure this endpoint exists or remove
            ]);
    ...
    const fetchEmployees = async (companyId: string) => {
        try {
            const res = await authFetch(`http://localhost:7000/api/employees?company_id=${companyId}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = initialData
                ? `http://localhost:7000/api/asos/${initialData.id}`
                : 'http://localhost:7000/api/asos';
            ...
            <div className="bg-gray-50 p-4 rounded-lg border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        >
                            <option value="">Selecione...</option>
                            {employees.map(e => <option key={e.id} value={e.id}>{e.name} ({e.cpf})</option>)}
                        </select>
                    </div>
                    <div>
